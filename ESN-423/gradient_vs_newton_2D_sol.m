% Practical 4 gradient_vs_newton_2D
clear all
close all
...
for ii=2:length(xvec)-1
    for jj=2:length(yvec)-1
        ...
        % we can use xvec_grad && yvec_grad also
        delzdelx(jj-1,ii-1) = (fx(xvec(ii-1)+delx,yvec(jj-1))- fx(xvec(ii-1)-delx,yvec(jj-1)))/(2*delx);
        delzdely(jj-1,ii-1) = (fx(xvec(ii-1),yvec(jj-1)+dely)- fx(xvec(ii-1),yvec(jj-1)-dely))/(2*dely);
...
% Hessian 
d2fdx2 = @(X,Y) (dfdx(X+delx, Y) - dfdx(X-delx, Y))/(2*delx);
d2fdy2 = @(X,Y) (dfdy(X, Y+dely) - dfdy(X, Y-dely))/(2*dely);
d2fdxdy = @(X,Y) (dfdy(X+delx, Y) - dfdy(X-delx, Y))/(2*delx);
d2fdydx = @(X,Y) (dfdx(X, Y+dely) - dfdx(X, Y-dely))/(2*dely);
