...
dgdk = @(k,v0,z) (-1)*(1/k^2)*(log(v0+k*z) - log(v0)) + (1/k)*(z./(k*z+v0));
dgdv0 = @(k,v0,z) (1/k)*(1./(v0+k*z) - (1/v0));
...
d2gdk2 = @(k,v0,z) (2/k^3)*(log(k*z+v0) - log(v0)) - (2/k^2)*(z./(k*z+v0)) - (1/k)*((z.^2)./((k*z+v0).^2));
d2gdv02 = @(k,v0,z) (-1/k)*(1/(k*z+v0).^2) + (1/k)*(1/v0.^2);
d2gdkdv0 = @(k,v0,z) (-1/k^2)*(1/k*z+v0) + (1/k^2)*(1/v0) - (1/k)*(z/(k*z+v0).^2);
d2gdv0dk = @(k,v0,z) (-1/k^2)*(1/k*z+v0) + (1/k^2)*(1/v0) - (1/k)*(z/(k*z+v0).^2); % same as d2Hdkdv0
...
for ii=1:n_iterataion
    ...
    % matrix K
    dm(:,ii) = inv(G'*G)*G'*(d_obs - gm(m1(ii),m2(ii),sensor_depth));
...
for ii=1:n_iterataion
    ...
    Ki(1,1,:) = d2gdk2(m1(ii),m2(ii),sensor_depth);
    Ki(1,2,:) = d2gdv02(m1(ii),m2(ii),sensor_depth);
    Ki(2,1,:) = d2gdkdv0(m1(ii),m2(ii),sensor_depth);
    Ki(2,2,:) = d2gdv0dk(m1(ii),m2(ii),sensor_depth);
